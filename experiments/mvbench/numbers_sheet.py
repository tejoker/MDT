...
def markdown_sheet():
    """Same numbers as the LaTeX sheet, as a README-linkable Markdown report.

    Exists so the README carries no numbers of its own: a hand-typed table in
    prose went stale within a day of the last fix.
    """
    by = table(load("all_metrics"))
    base = per_dataset(by, BASE)
    prr_base = per_dataset(by, "mdt_rand")
    ...
    for m in [k for _, g in GROUPS for k in g]:
        ...
        v = np.asarray([ind[d] for d in shared])
        eff = np.asarray([base[d] - ind[d] for d in shared])
        secs = [by[(d, s, m)]["train_seconds"] for d, s, k in by if k == m and d in shared]
        den = float(np.mean([prr_base[d] for d in shared if prr_base[d] is not None]))
        stats[m] = dict(ind=v.mean(), std=v.std(ddof=1),
                        test=float(np.mean([tst[d] for d in shared])),
                        prr=v.mean() / den if abs(den) > 1e-3 else None,
                        w=int((eff > 0).sum()), l=int((eff < 0).sum()),
                        p=sign_p(eff), sec=float(np.mean(secs)) if secs else None)
    adj = holm([(k, x["p"]) for k, x in stats.items() if k not in (BASE, "__oracle__")])
    plain = {k: (v.replace(r"\textsc{", "").replace(r"\emph{", "").replace("}", "")
                 .replace(r"\ ", " ").replace("$^{\\P}$", "").strip())
             for k, v in NAME.items()}
    for title, keys in GROUPS:
        lines.append(f"| **{title}** | | | | | | | | |")
        for m in keys:
            x = stats[m]
            wl = "-" if m == BASE else ("n/a" if m == "__oracle__" else f"{x['w']}-{x['l']}")
            ph = "-" if m in (BASE, "__oracle__") else fmt(adj.get(m), 3)
            lines.append(
                f"| {plain[m]} | {'yes' if m in ENCOMPASSED else 'no'} | "
                f"{'yes' if m in NATIVE else 'no'} | {x['ind']:.3f}+-{x['std']:.3f} | "
                f"{x['test']:.3f} | {fmt(x['prr'], 2)} | {wl} | {ph} | {fmt(x['sec'], 1)} |")
    lines += [
        "",
